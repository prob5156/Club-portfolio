...
if ($search) {
    $where .= " AND (label LIKE ? OR number_value LIKE ?)";
    $params = ["%$search%", "%$search%"];
}

$countStmt = $pdo->prepare("SELECT COUNT(*) FROM achievements WHERE $where");
$countStmt->execute($params);
$totalItems = $countStmt->fetchColumn();
$totalPages = ceil($totalItems / $perPage);

$query = "SELECT * FROM achievements WHERE $where ORDER BY display_order ASC, id DESC LIMIT $perPage OFFSET $offset";
...
<div class="content-area">
    ...
    <div class="dashboard-card" style="flex-direction: column; align-items: stretch; padding: 20px;">
        <form method="GET" action="" style="display: flex; gap: 15px; margin-bottom: 20px;">
            <input type="text" name="search" class="form-control" placeholder="Search achievements..." value="<?= htmlspecialchars($search) ?>" style="flex: 1;">
                            ...
                            <tr>
                                <td>
                                    <?php if ($item['icon_svg']): ?>
                                        <div style="width: 40px; height: 40px; color: var(--color-primary);">
                                            <svg viewBox="0 0 24 24" fill="currentColor"><?= $item['icon_svg'] ?></svg>
                                        </div>
                                    <?php else: ?>
                                        <span style="color:var(--color-text-muted);">None</span>
                                    <?php endif; ?>
                                </td>
                                <td style="font-size: 1.2rem; font-weight: bold;"><?= htmlspecialchars($item['number_value']) ?></td>
                                <td style="font-weight: 500;"><?= htmlspecialchars($item['label']) ?></td>
                                ...
                                <td>
                                    ...
                                    <a href="delete.php?id=<?= $item['id'] ?>" class="btn btn-secondary" style="padding: 6px 12px; font-size: 0.9rem; color: #ef4444; border-color: rgba(239, 68, 68, 0.3);" onclick="return confirm('Are you sure you want to delete this achievement?');">Delete</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="6" style="text-align: center; color: var(--color-text-muted);">No achievements found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
