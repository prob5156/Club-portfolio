...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name) {
        ...
        if (!$error) {
            try {
                $stmt = $pdo->prepare("UPDATE sponsors SET name = ?, logo_path = ?, website_url = ?, description = ?, display_order = ?, is_active = ? WHERE id = ?");
                $stmt->execute([$name, $logo_path, $website_url, $description, $display_order, $is_active, $id]);
                $_SESSION['toast'] = ['type' => 'success', 'message' => 'Sponsor updated successfully!'];
                header("Location: index.php");
                exit();
            } catch (Exception $e) {
                $error = "Failed to update sponsor: " . $e->getMessage();
            }
        }
    }
}
        ...
        <form method="POST" action="" enctype="multipart/form-data">
            <div style="display: grid; grid-template-columns: 1fr; gap: 20px;">
                ...
                <div class="form-group">
                    ...
                    <input type="url" name="website_url" class="form-control" value="<?= htmlspecialchars($item['website_url'] ?? '') ?>">
                </div>
                ...
                <div class="form-group">
                    ...
                    <?php if ($item['logo_path']): ?>
                        <div style="margin-bottom: 10px;">
                            <img src="/Dhrupodi/<?= htmlspecialchars($item['logo_path']) ?>" style="height: 60px; background: #fff; padding: 5px; border-radius: 4px;">
                        </div>
                    <?php endif; ?>
            ...
            <div style="margin-top: 30px;">
                <button type="submit" class="btn btn-primary" style="padding: 12px 30px; font-size: 1.1rem;">Update Sponsor</button>
            </div>
        </form>
    </div>
</div>
