...
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM sponsors WHERE $where");
$countStmt->execute($params);
$totalItems = $countStmt->fetchColumn();
$totalPages = ceil($totalItems / $perPage);

$query = "SELECT * FROM sponsors WHERE $where ORDER BY display_order ASC, id DESC LIMIT $perPage OFFSET $offset";
...
<div class="content-area">
    ...
    <div class="dashboard-card" style="flex-direction: column; align-items: stretch; padding: 20px;">
        <form method="GET" action="" style="display: flex; gap: 15px; margin-bottom: 20px;">
            <input type="text" name="search" class="form-control" placeholder="Search sponsors..." value="<?= htmlspecialchars($search) ?>" style="flex: 1;">
                            ...
                            <tr>
                                <td>
                                    <?php if ($item['logo_path']): ?>
                                        <img src="/Dhrupodi/<?= htmlspecialchars($item['logo_path']) ?>" alt="Logo" style="width: 60px; height: 60px; object-fit: contain; background: #fff; padding: 5px; border-radius: 4px; border: 1px solid rgba(255,255,255,0.1);">
                                    <?php else: ?>
                                ...
                                <td>
                                    <?php if ($item['website_url']): ?>
                                        <a href="<?= htmlspecialchars($item['website_url']) ?>" target="_blank" style="color:#3b82f6;">Link</a>
                                    <?php else: ?>
                                ...
                                <td>
                                    ...
                                    <a href="delete.php?id=<?= $item['id'] ?>" class="btn btn-secondary" style="padding: 6px 12px; font-size: 0.9rem; color: #ef4444; border-color: rgba(239, 68, 68, 0.3);" onclick="return confirm('Are you sure you want to delete this sponsor?');">Delete</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="6" style="text-align: center; color: var(--color-text-muted);">No sponsors found.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
