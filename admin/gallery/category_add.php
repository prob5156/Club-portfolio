...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$name) {
        ...
        if (!$error) {
            try {
                $stmt = $pdo->prepare("INSERT INTO gallery_categories (name, slug, description, cover_image_path, is_active, display_order) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$name, $slug, $description, $cover_image_path, $is_active, $display_order]);
    ...
    <div class="dashboard-card" style="flex-direction: column; align-items: stretch; padding: 30px;">
        ...
        <form method="POST" action="" enctype="multipart/form-data">
            ...
            <div style="margin-top: 30px;">
                <button type="submit" class="btn btn-primary" style="padding: 12px 30px; font-size: 1.1rem;">Save Category</button>
            </div>
        </form>
    </div>
</div>
